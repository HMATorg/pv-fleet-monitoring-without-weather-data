...
import os
SEED=int(os.environ.get('SEED',0)); torch.manual_seed(SEED); np.random.seed(SEED); torch.set_num_threads(2)
H=load(); L=48
def build(h):
    Xs,Xf,Y,meta=[],[],[],[]
...
pd.concat(out).to_parquet('out/preds_lstm_seed%d.parquet'%SEED+'')
