...
The study uses five and a half years of hourly production from ten municipal PV systems in Calgary, Canada (City of Calgary open data, dataset `ytdn-2qsp`) and no on-site or reanalysis weather inputs. It covers three tasks:
...
- **Fault detection.** A peer-referenced robust causal z-score with alarm masking and a silent-day rule, benchmarked on injected faults of six types against a single-site clear-sky index and Isolation Forest.
